...
public class LoginCheckInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		if(loginMember != null) { //로그인한 상태라면
			...
			//http://localhost:8969/second/blist.do?page=1
			String origin = request.getHeader("Origin");
			logger.debug("Origin : " + origin);
			//브라우저이름[전송방식]
			//http://localhost:8969, chrome[GET], ie[GET/POST], firefox[POST] => null
			...
			return false; //컨트롤러로 안가고 리턴하면, 뷰를 직접 지정함
		} //else : 로그인하지 않았다면
		
		return super.preHandle(request, response, handler); //기본이 true임, 해당 컨트롤러로 넘어감
	}
