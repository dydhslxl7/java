...
public class BeforeAdvice {
	...
	//어드바이스 클래스 안에서 포인트컷 설정을 직접 작성하고, 바로 위빙할 메소드에
	//바로 적용 테스트해 봄
	
	//private Logger logger = LoggerFactory.getLogger(BeforeAdvice.class);
	private Logger logger = LoggerFactory.getLogger(getClass()); // == this.getClass()
	
	//1. pointcut 설정
	@Pointcut("execution(* com.kh.second..*.*(..))")
	public void allPointcut() {}
	
	//2. 작동시점과 적용할 포인트컷을 지정함 : 실행시킬 내용에 대한 코드 작성 및 실행시점 설정함
	@Before("allPointcut()")
	public void beforeLog(JoinPoint jp) {
		//포인트컷으로 지정한 타겟오브제트의 메소드가 실행되기 전에 공통으로 작동될 기능에 대한 메소드임
